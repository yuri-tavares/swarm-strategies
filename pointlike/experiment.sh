...
defaultVarNames=(s    d    X  theta    y0  v   )
# default values for each variable
defaultValues=(  0.3 1.0 10.0  0.0   100 1.0 )
...
doExperiments(){
  ...
  while (( $(echo "scale=2;$varValue <= $maxValue" | bc -q) ));
  do
    ...
    varValue=`echo "scale=2;$varValue + $varInc" | bc -q`
  done
}
...
for (( i=0; i<$sizeVars; i++ )); do
  if [ ! -z $samples ]; then
    varInc=`echo "scale=2;(${maxExp[$i]}-${minExp[$i]})/(${samples[$i]} - 1)" | bc -q`
  else
    varInc=${increment[$i]}
  fi
