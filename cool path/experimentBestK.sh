...
defaultVarNames=(s    d   v   K   time)
# default values for each variable
defaultValues=(  3.0 1.0  0.1 10  70000000)
...
# Calculates the best K for given parameters and appends it in configuration file
...
addLineWithBestK(){
  ...
  local bestK=`python3 findBestK.py $s $d`
  echo "K="$bestK >> $configName
}
...
makeConfigFile(){
  ...
  addLineWithBestK $varName $varValue $configName 
