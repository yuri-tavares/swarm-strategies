...
vector<coord>* makeTriangularTiling(double s, double X, double d, double v){
  double Y = waypoints[0][1] - s;
  vector<coord> * L = new vector<coord>();
  double ds = v*0.1; //time for Stage change the velocity is 0.1 s.
  double dp, x0 = waypoints[0][0] + ds;
  coord p = {.x = x0, .y = Y, .theta = PI};
  int sign = -1;
  dp = (s <= sqrt(3)*d/4)? sqrt(d*d - 4*s*s) : d/2;
  while (p.x <= X + x0){
    L->push_back(p);
    sign *= -1; 
    p = {.x = p.x + dp, .y = p.y + sign*2*s, .theta = PI};
  }
  return L;
}
...
int main(int argc, char **argv)
{
  ...
  if (!(0 < s && s < d/2.)){
    cout << "Must have 0 < s < d/2. Exiting...";
    exit(1);
  }
  try{
  ...
  vector<coord> * positions = makeTriangularTiling(s, X, d, v);
  int numRobot = 0;
