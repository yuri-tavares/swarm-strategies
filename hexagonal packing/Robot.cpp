...
void Robot::readConfigFile(string const& confFileName){
  ...
  try{
    ...
    cout << "Robot.o: Configuration file is incorret: " << s << endl;
    exit(1);
  }
  try{
...
void Robot::mainLoop(){
  ...
  if ((pho() < waypointDist + epsilon) && !finished)
  {
    ...
    numIterations = 0;
    FinalLog::refresh(numIterationsReachGoal, numIterations, stalls, theWorld->SimTimeNow());
  ...
  /*// Code for checking time for accelarate from stationary pose.
  char strv[20];
  sprintf(strv,"%lu μs: Robot %d: ",theWorld->SimTimeNow(),m_id);
  this->pos->GetVelocity().Print(strv);*/
